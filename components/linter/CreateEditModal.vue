...
<EModal
    ref="modal"
    :title="tr((isEdit ? 'edit' : 'create') + '-linter')"
    :isLoading="isDoingRequest"
    @right-btn-clicked="() => {
        $store.dispatch(
            'cms/linters/createEditEntity',
            {
                hideModal: (entity) => {
...
export default {
	computed: {
        ...mapState('cms/linters', ['entity', 'isEdit', 'isDoingRequest']),
    },
	...
	methods: {
		...
		...mapMutations({
			setNewEditEntity: 'cms/linters/setNewEditEntity'
		}),
	},
};
