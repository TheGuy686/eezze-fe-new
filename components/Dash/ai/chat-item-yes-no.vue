...
>
    ...
    >
        <div>
            ...
            <div :class="`p-2 ${ isAi ? 'ai' : 'receiver' } ${!message.showAvatar ? 'mt-1' : ''}`">
...
export default {
    ...
    computed: {
        initials() {
            if (this.isAi) return 'E-AI';

            return this.nameInitials(this.message.userFullName);
        },
    },
    data() {
        return {
    ...
    async mounted() {
        ...
        else this.messageIn = this.message.message.replace(/\n/g, '<br>');
    },
    methods: {
        forceComplete() {
